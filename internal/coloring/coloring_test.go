...
func TestApplyColor_AdvancedCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := ApplyColor(tt.asciiArt, tt.text, tt.substring, colorCode, tt.charWidths)
			count := 0
...
func TestApplyColor_FinalCoverage(t *testing.T) {
	...
	t.Run("Empty_Inputs", func(t *testing.T) {
		if ApplyColor(nil, "a", "a", colorCode, []int{1}) != nil {
			t.Error("Nil art fail")
		}
		art := []string{"a"}
		if len(ApplyColor(art, "a", "a", colorCode, []int{})) != 1 {
			t.Error("Empty widths fail")
		}
		if len(ApplyColor(art, "", "a", colorCode, []int{1})) != 1 {
	...
	t.Run("Non_Contiguous", func(t *testing.T) {
		res := ApplyColor([]string{"abcabc"}, "abcabc", "a", colorCode, []int{1, 1, 1, 1, 1, 1})
	...
	t.Run("Mismatched_Lengths", func(t *testing.T) {
		res := ApplyColor([]string{"A"}, "AB", "A", colorCode, []int{1})
	...
	t.Run("Varying_Line_Lengths", func(t *testing.T) {
		art := []string{"ABC", "A"}
		res := ApplyColor(art, "ABC", "A", colorCode, []int{1, 1, 1})
		if len(res) != 2 {
	...
	t.Run("Edge_Transitions", func(t *testing.T) {
		res := ApplyColor([]string{"abc"}, "abc", "c", colorCode, []int{1, 1, 1})
		if !strings.HasSuffix(res[0], Reset) {
			t.Error("End transition fail")
		}
	})

	t.Run("Longer_Substring", func(t *testing.T) {
		res := ApplyColor([]string{"a"}, "a", "abc", colorCode, []int{1})
	...
	t.Run("Short_Art_Truncation", func(t *testing.T) {
		res := ApplyColor([]string{"12"}, "A", "A", colorCode, []int{10})
